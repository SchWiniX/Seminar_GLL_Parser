...
int base_loop(
	...
	gss_info->gss_edges[0].src_node = 0;
	gss_info->gss_edges[0].target_node = 1;
	...
	if(!setjmp(L0_jump_buf)) {
		...
		for(int i = 0; i < rule_info->rules[rule_info->rule - 65].first_size; i++) {
			...
			if('_' == rule_info->rules[rule_info->rule - 65].first[i] && input_info->input[input_info->input_idx] == '$') first_check = 1;
		}
		if(first_check) {
			init_rule(rule_info, input_info, gss_info, set_info);
		} else return 0;
	...
	if(r_size != 0) {
		struct descriptors curr_descriptor = set_info->R_set[--r_size];
